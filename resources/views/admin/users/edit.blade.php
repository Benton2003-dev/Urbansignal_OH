...
<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        <h1 class="text-xl font-bold text-gray-900 mb-6">Modifier : {{ $user->name }}</h1>
        ...
        <form action="{{ route('admin.users.update', $user) }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <select name="role" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500">
                    <option value="citizen" {{ $user->role==='citizen'?'selected':'' }}>Citoyen</option>
                    <option value="agent" {{ $user->role==='agent'?'selected':'' }}>Agent technique</option>
                    <option value="admin" {{ $user->role==='admin'?'selected':'' }}>Administrateur</option>
                </select>
            </div>
            <div class="flex items-center gap-3">
                <input type="checkbox" name="is_active" id="is_active" value="1" {{ $user->is_active?'checked':'' }} class="w-4 h-4 text-purple-600 rounded">
            ...
            <div class="flex justify-end gap-3 pt-2">
                ...
                <button type="submit" class="px-6 py-2.5 bg-purple-600 text-white font-semibold rounded-xl hover:bg-purple-700 transition">Sauvegarder</button>
            </div>
        </form>
    </div>
</div>
