...
<body class="bg-gray-50 font-sans antialiased">
    ...
    <nav class="bg-white shadow-sm border-b border-gray-200" x-data="{ mobileOpen: false }">
            ...
            <div class="flex justify-between h-16">
                ...
                <div class="flex items-center">
                    <a href="{{ route('home') }}" class="flex items-center space-x-2">
                        <div class="w-9 h-9 bg-green-600 rounded-lg flex items-center justify-center shadow-sm">
                ...
                <div class="hidden md:flex items-center space-x-1">
                    @auth
                        ...
                        @else
                            ...
                            <a href="{{ route('citizen.reports.create') }}" class="px-3 py-2 rounded-md text-sm font-medium text-green-700 hover:bg-green-50 transition">+ Signaler un problème</a>
                        ...
                        <div class="relative ml-2" x-data="{ open: false }">
                            <button @click="open = !open" class="flex items-center space-x-2 px-3 py-2 rounded-lg hover:bg-gray-100 transition">
                                <div class="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center">
                                    <span class="text-green-700 text-sm font-bold">{{ strtoupper(substr(auth()->user()->name, 0, 1)) }}</span>
                                </div>
                            ...
                            <div x-show="open" @click.away="open = false" x-transition
                                ...
                                <div class="px-4 py-3 border-b border-gray-100 bg-gray-50">
                                    ...
                                    <span class="inline-block mt-1 px-2 py-0.5 text-xs rounded-full font-medium
                                        {{ auth()->user()->isAdmin() ? 'bg-purple-100 text-purple-700' : (auth()->user()->isAgent() ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700') }}">
                                        {{ auth()->user()->isAdmin() ? 'Administrateur' : (auth()->user()->isAgent() ? 'Agent technique' : 'Citoyen') }}
                                    </span>
                                </div>
                    ...
                    @else
                        ...
                        <a href="{{ route('register') }}" class="ml-1 px-4 py-2 bg-green-600 text-white text-sm font-semibold rounded-lg hover:bg-green-700 transition shadow-sm">S'inscrire</a>
                    @endauth
                </div>
        ...
        <div x-show="mobileOpen" class="md:hidden border-t border-gray-200 bg-white px-4 py-3 space-y-1">
            ...
            @else
                ...
                <a href="{{ route('register') }}" class="block px-3 py-2 rounded-md text-sm font-medium text-green-700 hover:bg-green-50">S'inscrire</a>
            @endauth
        </div>
    </nav>
    ...
    @if(session('success'))
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4">
            <div class="flex items-center p-4 bg-green-50 border border-green-200 rounded-lg text-green-800 text-sm">
